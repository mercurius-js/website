...
async function processReleases(opts) {
  ...
  for (const docTree of getDocFolders(releasesFolder)) {
    ...
    const docDestination = join(
      versionedFolder,
      `version-${versionName}`,
      requiresRootFolder ? "Documentation" : ""
    );
  ...
  await writeJsonFile(join(webSiteRoot, "versions.json"), [
    latestVersionName,
    ...orderedVersions
  ]);
  ...
  const versionsShipped = (
    await fs.readFile(join(__dirname, `releases.tag`), "utf8")
  ).split("\n");
  await writeJsonFile(
  ...
  log.info(
    `Wrote %d versions to versions-shipped.json`,
    versionsShipped.length
  );
  ...
  for (const fileTree of releasesTree.children) {
    if (fileTree.type === "directory") {
      try {
        ...
        log.info(
          `Error processing release [${fileTree.name}] because ${err.message} `
        );
        console.error(err);
      }
    }
...
async function generateCategoriesFiles(docsDir) {
  const files = glob.sync(`${docsDir}/**/index.md`, {
    nodir: true,
    nocase: true
  });
  await Promise.all(
...
async function fixBrokenLinks(dir) {
  const fixes = [
    ...
    {
      regex: /<h1 style="text-align: center;">mercurius<\/h1>/g,
      replacement: ""
    }
  ];
